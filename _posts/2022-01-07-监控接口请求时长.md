...
date:       2026-01-07
author:     River
header-img: img/okhttp_listener/okhttp_lib_apm.png
catalog: true
tags:
    - OKhttp
    - Android
...
# 前言
>移动端线上监控体系包括网络请求的控件，Android上实际开发起来并不是一件容易的事情。
>ios因为有系统提供的NSURLProtocol，相对较容易实现；鸭梨山大。
![莫道桑榆晚，为霞尚满天](/img/okhttp_listener/okhttp_pipeline.png)
...
NetXXXBean.Builder builder = NetXXXBean.Builder
        ...
        parseProbeEvent(builder, ProbeEventListener.getEventEntity(request));
        APMEventRecorder.getInstance().saveHBNet(builder.build());
```
